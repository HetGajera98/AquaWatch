...
- Collects **real-time telemetry** from custom ESP32-S3 IoT nodes (tank level, flow rate, float-switch status)
...
| **Database** | PostgreSQL |
| **IoT Layer** | ESP32-S3 nodes over Wi-Fi → Blynk Cloud |
...
Frontend (React 18 + TS)  <->  Node.js Backend (Express, Prisma, JWT)  <->  Python AI Microservice (FastAPI)
        |                              |         ^                                  
        v                              v         |                                  
  Browser State                  PostgreSQL   Blynk Cloud  <-- Wi-Fi --  ESP32-S3 Node
...
| Component | Role |
|---|---|
| **ESP32-S3** | Wi-Fi microcontroller node — the brain of each rig |
...
> Requires PostgreSQL running locally, a Blynk project/device template configured for your ESP32-S3 node(s), and a weather API key (OpenWeatherMap or Open-Meteo).
...
| **DevOps / IoT Engineer** | Database, ESP32-S3 firmware, Blynk setup, deployment |

## Project Scope (MVP)

**Must Ship**
- Login / Signup
- 1+ live ESP32-S3 node via Blynk
